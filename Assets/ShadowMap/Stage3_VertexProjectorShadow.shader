...
1.把顶点朝一个方向压扁到地面上模拟试试阴影的效果
*********************************************************************/


Shader "Mya/VertexProjectorShadow"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
            ...
            float3 ShadowProjectPos(float4 vertDir)
            {
                ...
                //得到顶点的世界空间坐标
                float3 wPos = mul(_Object2World , vertDir).xyz;
                ...
                shadowPos.xz = wPos.xz - lightDir.xz * (wPos.y/lightDir.y) ;
			...
			v2f vert (appdata v)
			{
                ...
                o.vertex = mul(UNITY_MATRIX_MVP,shadowPos);
                //得到中心点世界坐标
